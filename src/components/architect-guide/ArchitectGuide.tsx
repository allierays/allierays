...
import { OBJECTIVE_VISUALS } from './objectiveVisuals';
import { OBJECTIVE_REVIEWS } from './objectiveReviews';
import { OBJECTIVE_LESSONS, type ObjectiveLesson } from './objectiveLessons';
...
function ObjectivePage({ objectiveId, go }: { objectiveId: string; go: (page: Page) => void }) {
  ...
  return <div className="w-full min-w-0">
    ...
    <p className="mt-2 text-[16px] font-extrabold leading-relaxed" style={{ color: meta.color }}>{spec.question}</p>
    <p className="mt-1 max-w-3xl text-[15px] leading-relaxed text-[#394646]">{spec.explain}</p>

    <div className="mt-5 space-y-3">
      <TeachingLesson lesson={lesson} color={meta.color} bg={meta.bg} />

      <ObjectiveGraphic id={objectiveId} spec={spec} color={meta.color} bg={meta.bg} />

      {objectiveId === '1.4' && <MultiAgentBlueprint />}

      <Frame title={spec.lensTitle} color={meta.color}>
        <p className="mb-3 text-[12px] leading-relaxed text-[#637979]">{lesson.lensIntro}</p>
        <div className={`grid gap-2 ${spec.lenses.length === 3 ? 'sm:grid-cols-3' : 'sm:grid-cols-2'}`}>
          {spec.lenses.map(([title, text]) => <Box key={title} title={title} text={text} />)}
        </div>
      </Frame>

      <Frame title="Read the scenario" color={meta.color}>
        <div className={`grid gap-2 ${scenarioColumns}`}>
          {review.scenarios.map(([clue, move, why]) => <div key={clue} className="overflow-hidden rounded-lg border border-[#DED8CE] bg-white">
            <div className="bg-[#FAF9F4] px-3 py-2 text-[10px] font-bold uppercase tracking-[0.08em] text-[#637979]">When the scenario says</div>
            <div className="px-3 py-2.5">
              <p className="text-[12px] font-extrabold text-[#394646]">{clue}</p>
              <p className="mt-2 text-[11px] font-extrabold" style={{ color: meta.color }}>Choose: {move}</p>
              <p className="mt-1 text-[11px] leading-snug text-[#637979]">{why}</p>
            </div>
          </div>)}
        </div>
      </Frame>

      <Frame title="Tempting answers that miss the point" color={meta.color}>
        <div className={`grid gap-2 ${trapColumns}`}>
          {review.traps.map(([temptation, correction]) => <div key={temptation} className="grid grid-cols-[auto_1fr] gap-2 rounded-lg border border-[#F3CACA] bg-[#FFF7F7] px-3 py-2.5">
            <span className="font-extrabold text-[#B91C1C]">×</span>
            <div><p className="text-[12px] font-extrabold text-[#8D3030]">{temptation}</p><p className="mt-1 text-[11px] leading-snug text-[#637979]">{correction}</p></div>
          </div>)}
        </div>
      </Frame>

      <Rule color={meta.color} bg={meta.bg}>{spec.rule}</Rule>
      {review.sources && <div className="flex flex-wrap items-center gap-2 rounded-lg bg-[#FAF9F4] px-3 py-2 text-[10px]">
        <span className="font-bold uppercase tracking-[0.08em] text-[#9A9389]">Go deeper</span>
        {review.sources.map(([label, url]) => <a key={url} href={url} target="_blank" rel="noreferrer" className="rounded-full border border-[#DED8CE] bg-white px-2.5 py-1 font-bold text-[#35656E] hover:underline">{label} ↗</a>)}
      </div>}
    </div>
    ...
    <div className="mt-5 flex items-stretch justify-between gap-2 border-t border-[#E6E1D8] pt-4">
      ...
      <button type="button" onClick={() => go(next)} className="rounded-lg border border-[#DED8CE] bg-white px-3 py-2 text-right text-[12px] font-bold text-[#394646] hover:border-[#35656E]">{index === domain.objectives.length - 1 ? (domainId === 7 ? 'Start' : `Domain ${domainId + 1} overview`) : domain.objectives[index + 1].id} →</button>
    </div>
  </div>;
}
