...
function ObjectiveRibbon({ id, go }: { id: DomainId; go: (page: Page) => void }) {
  const objectives = objectivesFor(id);
  const meta = metaById(id);
  return <div className="rounded-xl border border-[#DED8CE] bg-[#FAF9F4] p-4"><div className="flex flex-wrap items-baseline justify-between gap-2"><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#637979]">Visual pages in this domain</p><p className="text-[10px] font-bold text-[#9A9389]">{objectives.length} objectives</p></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{objectives.map((objective) => <button type="button" onClick={() => go(objective.id)} key={objective.id} className="group flex gap-2 rounded-lg bg-white px-3 py-2 text-left text-[12px] leading-snug text-[#394646] hover:ring-2 hover:ring-[#DED8CE]"><span className="flex-shrink-0 font-extrabold" style={{ color: meta.color }}>{objective.id}</span><span className="group-hover:underline">{objective.title}</span><span className="ml-auto" aria-hidden>→</span></button>)}</div></div>;
}
...
function ObjectivePageLegacy({ objectiveId, go }: { objectiveId: string; go: (page: Page) => void }) {
  ...
  const objectives = objectivesFor(domainId);
  const objective = objectives.find((item) => item.id === objectiveId)!;
  const meta = metaById(domainId);
  const index = objectives.findIndex((item) => item.id === objectiveId);
  const previous = index === 0 ? `domain-${domainId}` : objectives[index - 1].id;
  const next = index === objectives.length - 1 ? (domainId === 7 ? 'start' : `domain-${(domainId + 1) as DomainId}`) : objectives[index + 1].id;
...
function ObjectivePage({ objectiveId, go }: { objectiveId: string; go: (page: Page) => void }) {
  ...
  const index = objectives.findIndex((item) => item.id === objectiveId);
  const previous = index === 0 ? `domain-${domainId}` : objectives[index - 1].id;
  const next = index === objectives.length - 1
    ? (domainId === 7 ? 'start' : `domain-${(domainId + 1) as DomainId}`)
    : objectives[index + 1].id;
  return <div className="w-full min-w-0">
    ...
    <div className="mt-5 flex items-stretch justify-between gap-2 border-t border-[#E6E1D8] pt-4">
      <button type="button" onClick={() => go(previous)} className="rounded-lg border border-[#DED8CE] bg-white px-3 py-2 text-left text-[12px] font-bold text-[#394646] hover:border-[#35656E]">← {index === 0 ? `Domain ${domainId} overview` : objectives[index - 1].id}</button>
      <button type="button" onClick={() => go(next)} className="rounded-lg border border-[#DED8CE] bg-white px-3 py-2 text-right text-[12px] font-bold text-[#394646] hover:border-[#35656E]">{index === objectives.length - 1 ? (domainId === 7 ? 'Start' : `Domain ${domainId + 1} overview`) : objectives[index + 1].id} →</button>
    </div>
  </div>;
}
...
function ArchitectGuideLegacy() {
  ...
  const currentDomain = page === 'start' ? null : Number(page.startsWith('domain-') ? page.slice(7) : page.split('.')[0]) as DomainId;
  ...
  return <>
    <nav aria-label="Guide contents" className="hidden lg:flex flex-col gap-1">
      <button type="button" onClick={() => go('start')} aria-pressed={page === 'start'} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-left ${page === 'start' ? 'bg-[#FFF4CE]' : 'hover:bg-[#FAF9F4]'}`}>
        ...
        <span><span className="block text-sm font-extrabold text-[#394646]">Start here</span><span className="block text-xs text-[#637979]">38 objectives</span></span>
      </button>
      {META.map((domain) => {
        ...
        return <div key={domain.id}>
          ...
          {active && <div className="ml-6 border-l border-[#DED8CE] py-1 pl-3">
            {objectivesFor(domain.id).map((objective) => <button key={objective.id} type="button" onClick={() => go(objective.id)} aria-pressed={page === objective.id} className={`block w-full rounded-lg px-2 py-1.5 text-left text-[11px] leading-snug ${page === objective.id ? 'bg-white font-extrabold shadow-sm' : 'text-[#637979] hover:bg-white/70'}`}>
